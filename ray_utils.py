...
def volumetric_rendering(rgb, density, t_vals, dirs, white_bkgd,return_tquantile = False):
    ...
    if return_tquantile:
        ind1 = torch.argmax((torch.cumsum(density.squeeze(2), dim=1)/torch.sum(density.squeeze(2), dim=1)[:,None]>1e-5).int(), dim=1)
        t_p1 = t_mids[range(t_mids.shape[0]),ind1]
        ind2 = torch.argmax((torch.cumsum(density.squeeze(2), dim=1)/torch.sum(density.squeeze(2), dim=1)[:,None]>0.95).int(), dim=1)
        t_p2 = t_mids[range(t_mids.shape[0]),ind2]
        return comp_rgb, distance, acc, weights, alpha, t_p1, t_p2
