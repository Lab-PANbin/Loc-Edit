...
def get_dataset(dataset_name, base_dir, split, factor=4, device=torch.device("cpu"), train_ad_nerf = False,ori_image_num = None):
    if dataset_name=='ad_llff':
        d = dataset_dict[dataset_name](base_dir, split, factor=factor, device=device, train_ad_nerf=train_ad_nerf,ori_image_num = ori_image_num)
        return d,d.rays,d.images
    else:
        d = dataset_dict[dataset_name](base_dir, split, factor=factor, device=device, train_ad_nerf=train_ad_nerf)
        return d


def get_dataloader(dataset_name, base_dir, split, factor=4, batch_size=None, shuffle=True, device=torch.device("cpu"),train_ad_nerf = False,ori_image_num = None):
    if dataset_name=='ad_llff':
        d,diff_rays,diff_images = get_dataset(dataset_name, base_dir, split, factor, device, train_ad_nerf,ori_image_num = ori_image_num)
    else:
        d = get_dataset(dataset_name, base_dir, split, factor, device, train_ad_nerf)
    ...
    loader.far = d.far
    if dataset_name=='ad_llff':
        return loader,diff_rays,diff_images
    else:
        return loader

def render_train_dataloader(dataset_name, base_dir, split, factor=4, batch_size=None, shuffle=True, device=torch.device("cpu")):
    if dataset_name=='ad_llff':
        d = get_dataset(dataset_name, base_dir, split, factor, device)[0]
    else:
...
class NeRFDataset(Dataset):
    def __init__(self, base_dir, split, spherify=False, near=2, far=6, white_bkgd=False, 
                factor=1, n_poses=120, radius=None, radii=None, h=None, w=None, 
                device=torch.device("cpu"),train_ad_nerf = False,ori_image_num = None):
        ...
        self.rays = None
        self.images = None
        self.train_ad_nerf = train_ad_nerf 

        self.ori_image_num = ori_image_num
        self.load()
    ...
    def load(self):
        ...
        if self.train_ad_nerf:
            print('train_ad_nerf: load diff pixel region data')
            """        
            ...
            print('load the second pose of the data as reference pose (first training pose)')

            self.rays = Rays(
            origins=self.rays.origins[0][self.diff[:,:,None].repeat(1,1,3)].reshape(-1,3),
            directions=self.rays.directions[0][self.diff[:,:,None].repeat(1,1,3)].reshape(-1,3),
            viewdirs=self.rays.viewdirs[0][self.diff[:,:,None].repeat(1,1,3)].reshape(-1,3),
            radii=self.rays.radii[0][self.diff[:,:,None]].reshape(-1,1),
            lossmult=self.rays.lossmult[0][self.diff[:,:,None]].reshape(-1,1),
            near=self.rays.near[0][self.diff[:,:,None]].reshape(-1,1),
            far=self.rays.far[0][self.diff[:,:,None]].reshape(-1,1))

            self.images = self.images[self.diff[:,:,None].repeat(1,1,3)]
        """
        else:
            print(11)
            self.rays = Rays(
            origins=self.rays.origins[0].reshape(-1,3),
            directions=self.rays.directions[0].reshape(-1,3),
            viewdirs=self.rays.viewdirs[0].reshape(-1,3),
            radii=self.rays.radii[0].reshape(-1,1),
            lossmult=self.rays.lossmult[0].reshape(-1,1),
            near=self.rays.near[0].reshape(-1,1),
            far=self.rays.far[0].reshape(-1,1))
        """
...
class LLFF(NeRFDataset):
    def __init__(self, base_dir, split, factor=4, spherify=False, near=0, far=1, white_bkgd=False, device=torch.device("cpu"),train_ad_nerf = False):
        super(LLFF, self).__init__(base_dir, split, spherify=spherify, near=near, far=far, white_bkgd=white_bkgd, factor=factor, device=device, train_ad_nerf=train_ad_nerf)
    ...
    def generate_training_poses(self):
        ...
        for img_file in img_files:
            with open(img_file, 'rb') as img_in:
                print('loading img: {}'.format(img_file))
                image = to_float(np.array(Image.open(img_in)))
                images.append(image)
        images = np.stack(images, -1)  ##H W 3 img_num

        # Load poses
        with open(path.join(self.base_dir, 'poses_bounds.npy'), 'rb') as fp:
            poses_arr = np.load(fp)
        poses = poses_arr[:, :-2].reshape([-1, 3, 5]).transpose([1, 2, 0])  ##3 5 img_num
    ...
    def generate_training_rays(self):
        ...
        if self.train_ad_nerf:
            indices = [i for i in np.arange(20) if i not in np.arange(20)[::8]]  ##训练原nerf的图片数
...
class ad_LLFF(NeRFDataset):
    def __init__(self, base_dir, split, factor=4, spherify=False, near=0, far=1, white_bkgd=False, device=torch.device("cpu"),train_ad_nerf = False,ori_image_num = None):
        super(ad_LLFF, self).__init__(base_dir, split, spherify=spherify, near=near, far=far, white_bkgd=white_bkgd, factor=factor, device=device, train_ad_nerf=train_ad_nerf,ori_image_num = ori_image_num)
    ...
    def generate_training_poses(self):
        ...
        save_diff = np.any((np.abs(images[:,:,:,0] - images[:,:,:,1])>0.08),axis=2).astype(int)* 255
        import cv2
        cv2.imwrite('bool_visualization.png', save_diff)
        self.diff = torch.any(torch.tensor((np.abs(images[:,:,:,0] - images[:,:,:,1])>0.08)), dim=2)
    ...
    def generate_training_rays(self):
        print("Loading Training Poses")
        self.generate_training_poses()
        print(self.ori_image_num)
        if self.split == "train":
            indices = [i for i in np.arange(self.images.shape[0]) if i not in np.arange(self.images.shape[0])[::8]]
        else:
            indices = np.arange(self.images.shape[0])[::8]
        if self.train_ad_nerf:
            indices = [i for i in np.arange(self.ori_image_num) if i not in np.arange(self.ori_image_num)[::8]]  ##训练原nerf的图片数
            self.images = self.images
            self.poses = self.poses[indices]
        else:
            self.images = self.images[indices]
